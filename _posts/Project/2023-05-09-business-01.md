...
```python
# 고객 id를 기준으로 그룹화하고, 고객별로 총 구매액, 첫 주문일, 마지막 주문일, 총 주문횟수를 불러온다.
customer_df = df.groupby(by='Customer ID').agg({'Sales':['sum'], 
                                                'Profit': ['sum'],
                                                'Order Date':['min', 'max', 'nunique']})
# 고객 id를 기준으로 그룹화되어있는 것을 컬럼으로 내보낸다.
customer_df.reset_index(inplace=True)

# 컬럼명을 알아보기 좋게 설정한다.
customer_df.columns = ['customer_id', 'total_sales', 'total_profit',
                       'first_order_date', 'last_order_date', 'cnt_orders']
```
