...
resample 함수를 사용하면 날짜 데이터를 특정 규칙(특정 주기)을 기준으로 구간별 집계가 가능하다. rule에 'M'을 전달하면 날짜가 해당 월의 마지막날짜로 변환되고 월별 마지막 일자를 기준으로 월별 총 판매량을 집계하였다. 월별 총 판매량 그래프를 보면 판매되는 제품의 수가 전반적으로 증가하는 것으로 보인다.
